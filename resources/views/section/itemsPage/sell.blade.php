...
<div class="bg-gradient-to-r from-[#9e3636] to-[#821111]  min-h-screen w-full">
    ...
    <div class="flex flex-col w-full p-6 z-50">
        ...
        @if (session('status'))
            <div
                class=" bg-green-200 text-green-900 rounded-md p-4 fixed top-10 left-1/2 transform -translate-x-1/2 z-[60] opacity-100 pointer-events-auto transition-opacity duration-300 status-message">
                {{ session('status') }}
            </div>
        @endif
